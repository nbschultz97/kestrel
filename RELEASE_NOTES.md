...
- Offline city search and MGRS/latitude-longitude entry. Enter and result
  ...
  **Fly here** or **Build here** explicitly.
- Major-city labels and a lightweight Natural Earth land/coastline overview at
  zoom levels 3-8. This is not offline street mapping or faster imagery downloads.
- Free Flight C2 navigation and **Settings > MAP CONTROLS** for the MapToggle
  action: keyboard and digital gamepad/radio buttons, not a complete binding suite
  or radio-axis setup. Default map key: **F6**.
- Corrected release-owned 5-, 7- and 10-inch starter aircraft. The packaged
  7-inch starter now passes exact LAN aircraft approval. Selected-aircraft
  predictions use that aircraft, not the editing scratch build.
- The earlier 0.42.9 ten-inch compatibility and click-validation fixes remain
  included. This package repeats tests of six complete assemblies: three exact
  HQProp 10-inch products, each on CX10 ECO and Nazgul XL10 V6 frames with
  BrotherHobby Avenger 3110 730KV motors. This does not certify every catalog pair.
- Retains the targeted blank motor-thumbnail repair and visible-category
  prioritization. A general thumbnail loading-speed improvement is not established.
- DPI-aware search controls and honest **SOLAR** time / **SEASONAL** weather
  labels; these are not observed current weather or civil-time/DST calculations.

The intended public Cesium configuration is bundled; terrain streaming still
requires Internet access. No reporting endpoint is bundled. Existing personal
reporting configuration is not erased by this package.

## Verification

- 2,258 source tests plus 1,042 subtests; 24 native regression tests.
- Full Editor/Game build and fresh Unreal cook passed. Both aircraft compiler
  chains use precise floating-point semantics. All 103 launcher tests passed.
- All 4,447 archived files and the public configuration independently verified
  remotely and after local download/extraction.
- Both packaged starter tests and all three packaged prop-compatibility tests
  passed with protected inputs unchanged.
- Five navigation captures each at 720p and 1440p passed, including no automatic
  launch. City labels, overview layer and adjusted pin visually reviewed.
- All 13 Work Bench captures passed; selected and predicted Vandal weight both
  show 584 g. Settings capture confirms the MAP CONTROLS entry.

## Still open

Full action rebinding, physical controller/radio acceptance, physical two-PC LAN
play, general thumbnail latency and the broader model/texture/UI work remain
unfinished. Settings values still use ellipses at 720p. The Settings capture does
not validate interactive rebinding or in-flight physical input.

Street-address/POI lookup and live weather are not enabled. The overview layer
is intentionally coarse; detailed imagery still depends on its online provider.

Legacy-save caution: older 7-inch saves generated by the previous Game numeric
settings may fail exact LAN approval. Source review indicates that editing and
re-saving recomputes their contract, but this migration has not been runtime
validated. Preserve original saves; backward/forward LAN compatibility is not
certified. New bundled starters pass the packaged approval tests.

Stable Latest and the updater remain on **v0.42.4-alpha-unsigned.1**. This is a
separate-folder evaluation, not a claim that every roadmap feature has shipped.

Source: `37913bc20e17c74c5eea1b2830229cf7bca8e51b`

Archive: `KESTREL-alpha-win64.zip` (676,426,108 bytes)

SHA-256: `FC5BF97FAE7A746285D1846F99CD1CA73FA6EFF9FD3EEE98AE5CE44117CB3E2A`
